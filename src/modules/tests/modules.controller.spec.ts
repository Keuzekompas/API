...
describe('ModulesController', () => {
    ...
    describe('findOne', () => {
        ...
        it('should throw NotFoundException with status 404 if module not found', async () => {
            ...
            jest.spyOn(modulesService, 'findOne').mockRejectedValue(new NotFoundException('Module not found'));

            try {
                await modulesController.findOne('1');
            } catch (error) {
                console.log('Test: ModulesController.findOne (Error) - Caught Error:', error.message);
